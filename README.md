...
The **GM Screen | Encounter Roll Debug Mode** setting is disabled by default. Roll Zone Journal pages use a readable encounter-report layout with the selected table and supporting tables separated into category groups, including one group for all Trap tables and another for all Hazard tables. When enabled, the report includes dice formulas, roll totals, and result numbers; otherwise it retains the encounter context and all table result text, including Trap and Hazard results, while hiding dice, roll details, and result numbers.
